(()=>{
  ...
  ready(()=>{
    ...
    if(file==='post.html'){
      ...
      const render=()=>{
        const s=state(),rail=$('.revisionCycle'),a=$('#revisionAction');if(rail)rail.innerHTML=cycleHTML(s);
        ...
        if(a){
          ...
          else if(s.v3ChangesRequested&&(s.feedbackResolved||0)<3){a.className='revisionAction';a.innerHTML=`<div><small>REVISION INPUT</small><b>V3 · Changes Requested</b><span>当前已处理 ${s.feedbackResolved||0} / 3 条正式 Feedback。</span></div>`}
          ...
          else if(s.v4Draft&&!s.v4Submitted&&!s.v4ChangesRequested){a.className='revisionAction';a.innerHTML='<div><small>NEXT VERSION EVENT</small><b>正式提交 V4</b><span>Draft 对客户不可见；Submit 后才进入 Client Review。</span></div>'}
