(() => {
  ...
  function normalize(saved={}){
    ...
    if (!Array.isArray(saved.deliveryItems) || saved.deliveryItems.length !== 4) {
      if (saved.delivered || saved.archived || saved.deliveryRecord || saved.archiveRecord) {
        next.deliveryItems = [true,true,true,true];
      } else {
        next.deliveryItems = [false,false,false,false];
      }
    } else {
      next.deliveryItems = saved.deliveryItems.map(Boolean);
    }
