(()=>{
  ...
  ready(()=>{
    ...
    function nextAction(s){
      ...
      if(!s.workingComposite)return {href:'post.html',label:'建立 Working Composite',detail:'把实拍、AIGC 和 CG/Post 素材合成内部工作状态。'};
      if((s.feedbackResolved||0)<3)return {href:'post.html',label:'处理 V3 客户反馈',detail:`还有 ${3-(s.feedbackResolved||0)} 条反馈未闭合。`};
      if(!s.v4Draft)return {href:'post.html',label:'生成 V4 Draft',detail:'修改闭合后再生成正式版本草稿。'};
      if(!s.v4Submitted)return {href:'post.html',label:'提交 V4 给客户',detail:'提交后 V4 才进入 In Review。'};
      if(!s.approval)return {href:'review.html?stage=versions',label:'等待 / 获取 Version Approval',detail:'后期不能替客户创建 Approval。'};
    ...
    function buildInspector(){
      ...
      const drawer=document.createElement('aside');drawer.className='shotInspector';drawer.setAttribute('aria-label','SHOT 08 生产记录');drawer.innerHTML=`
        <div class="shotInspectorHead">
          <div><div class="siEyebrow">SHOT OBJECT · SHARED CONTEXT</div><h2>SHOT 08 · Hero Reveal</h2><p>同一个 Shot 在导演、实拍、AIGC、后期与客户审阅之间保持同一身份。</p></div>
          <button class="siClose" type="button" aria-label="关闭">×</button>
        </div>
      ...
      overlay.addEventListener('click',close);drawer.querySelector('.siClose').addEventListener('click',close);
      window.addEventListener('keydown',e=>{if(e.key==='Escape')close()});
      window.ReelOpsShotInspector={open,close,render:renderInspector};
    }

    function renderInspector(){
      const drawer=document.querySelector('.shotInspector');if(!drawer)return;
      const s=state();
      const lineage=[
        ['Creative',creativeText(s),s.creativeApproval,s.creativeSubmitted&&!s.creativeApproval],
      ...
      const events=[
        ...
        ['Creative Direction '+(s.creativeVersion||'V2'),s.creativeApproval?'客户已确认':s.creativeSubmitted?'已发布客户':'内部草稿',s.creativeSubmitted||s.creativeApproval,s.creativeApprovalAt?fmt(s.creativeApprovalAt):''],
    ...
    if(internalFiles.includes(file)&&!document.querySelector('.projectContextBar')){
      ...
      if(header){
        ...
        bar.querySelector('.pcbShotButton').addEventListener('click',()=>window.ReelOpsShotInspector?.open?.());
        const renderContext=()=>{
          const s=state();
