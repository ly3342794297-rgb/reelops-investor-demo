(()=>{
  ...
  ready(()=>{
    ...
    if(file==='director.html'){
      ...
      const creativeState=$('#creativeState');if(creativeState&&!$('.creativeStatusRail')){const rail=make('div','creativeStatusRail');creativeState.closest('.stateBlock')?.insertAdjacentElement('afterend',rail);const render=()=>{const s=state();rail.innerHTML=`<span class="${s.creativeSubmitted?'done':'active'}">Internal Draft</span><span class="${s.creativeSubmitted&&!s.creativeApproval&&!s.creativeChangesRequested?'active':s.creativeApproval?'done':''}">Client Review</span><span class="${s.creativeApproval?'done':s.creativeChangesRequested?'active':''}">Creative Approval</span>`;versionSection?.classList.toggle('isSuppressed',!s.v3Submitted&&!s.v3ChangesRequested&&!s.v4Draft&&!s.v4Submitted&&!s.approval)};render();window.addEventListener('reelops:state',render)}
    ...
    if(file==='review.html'){
      ...
      if(stageWrap?.parentElement&&!$('.clientProgress')){const progress=make('div','clientProgress');stageWrap.parentElement.appendChild(progress);const render=()=>{const s=state(),hasVersion=!!(s.v3Submitted||s.v3ChangesRequested||s.v4Draft||s.v4Submitted||s.v4ChangesRequested||s.approval),delivered=!!(s.deliveryRecord||s.delivered),versionState=s.approval?`${s.approvedVersion||'V4'} Approved`:s.v4ChangesRequested?'V4 Changes Requested':s.v4Submitted?'V4 In Review':s.v3ChangesRequested?'V3 Changes Requested':s.v3Submitted?'V3 In Review':'Not submitted';progress.innerHTML=`<div class="step ${s.creativeApproval?'done':s.creativeSubmitted?'current':''}"><b>01 创意审阅</b><span>${s.creativeApproval?'Creative Approved':s.creativeSubmitted?'Waiting for decision':'Not published'}</span></div><div class="step ${s.creativeApproval&&!hasVersion?'current':hasVersion?'done':'locked'}"><b>02 制作中</b><span>${s.creativeApproval?'Direction locked':'After creative approval'}</span></div><div class="step ${hasVersion&&!s.approval?'current':s.approval?'done':'locked'}"><b>03 成片审阅</b><span>${versionState}</span></div><div class="step ${s.approval&&!delivered?'current':delivered?'done':'locked'}"><b>04 最终交付</b><span>${delivered?'Delivered':s.approval?'Ready for delivery':'After version approval'}</span></div>`};render();window.addEventListener('reelops:state',render)}
