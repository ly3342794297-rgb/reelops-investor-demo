(()=>{
  ...
  ready(()=>{
    ...
    if(file==='director.html'){
      ...
      const submit=$('#submitCreative');if(submit)submit.onclick=e=>{e.preventDefault();const s=state();if(s.creativeSubmitted&&!s.creativeApproval&&!s.creativeChangesRequested){location.href='review.html?stage=creative';return}if(s.creativeChangesRequested||s.creativeApproval){const v=`V${(parseInt(String(s.creativeVersion||'V2').replace(/\D/g,''),10)||2)+1}`;set({creativeVersion:v,creativeSubmitted:true,creativeSubmittedAt:new Date().toISOString(),creativeApproval:false,creativeApprovalAt:null,creativeChangesRequested:false,newCreativeFeedback:0});const t=$('#toast');if(t){t.textContent='Creative Direction '+v+' 已作为新版本提交客户审阅';t.classList.add('show')}setTimeout(()=>location.href='review.html?stage=creative',480);return}set({creativeSubmitted:true,creativeSubmittedAt:new Date().toISOString(),creativeApproval:false,creativeApprovalAt:null,creativeChangesRequested:false,newCreativeFeedback:0});const t=$('#toast');if(t){t.textContent='Creative Direction '+(s.creativeVersion||'V2')+' 已发布给客户';t.classList.add('show')}setTimeout(()=>location.href='review.html?stage=creative',480)};
    }
