(()=>{
  ...
  ready(()=>{
    ...
    function project(s){
      ...
      const health=$('#healthGrid');if(health){const versionState=s.approval?[`good`,`${approved} Approved`,'Version Approval Record 已形成']:s.v4ChangesRequested?['attn','V4 Changes','下一正式版本应创建 V5']:s.v4Submitted?['wait','V4 In Review','等待客户版本决策']:s.v4Draft?['attn','V4 Draft','尚未正式提交']:s.v3ChangesRequested?['attn','V3 Changes','Revision 正在处理']:s.v3Submitted?['wait','V3 In Review','等待客户首次版本决策']:s.workingComposite?['attn','Composite','内部工作状态']:['','Not ready','尚未形成正式 Version'];const creative=s.creativeApproval?['good','Approved',`${s.creativeVersion||'V2'} 已锁定`]:s.creativeSubmitted?['wait','Waiting','等待客户创意决策']:['attn','Not published','创意方向尚未发布'];const capture=s.aiReady?['good','AI Ready',s.packageSent?'AI Production Package 已发送':'现场输入已齐套']:s.creativeApproval?['attn','Missing inputs','AI Ready 尚未闭合']:['','Locked','等待 Creative Approval'];const d=s.archiveRecord?['good','Archived','项目记录已闭环']:s.deliveryRecord?['good','Delivered','Delivery Record 已完成']:s.finalMasterReady?['attn',`${s.deliverables||0}/4 Ready`,'Deliverables 尚未闭合']:s.approval?['attn','Final Master Pending',`${approved} Approved → Final Master`]:['','Locked','等待 Version Approval'];health.innerHTML=[["CREATIVE",...creative],["AI READY",...capture],["VERSION",...versionState],["DELIVERY",...d]].map(x=>`<div class="health ${x[1]}"><div class="k">${x[0]}</div><strong>${x[2]}</strong><span>${x[3]}</span></div>`).join('')}
    ...
    function review(s){
      ...
      const hasVersion=!!(s.v3Submitted||s.v3ChangesRequested||s.v4Submitted||s.v4ChangesRequested||s.approval),formal=s.approval?(s.approvedVersion||'V4'):(s.v4Submitted||s.v4Draft||s.v4ChangesRequested?'V4':s.v3Submitted?'V3':null);
      $$('.stageBtn').forEach(b=>{if(b.dataset.stage==='versions')b.style.opacity=hasVersion?'1':'.58'});
      const screen=$('#versionsStage .reviewScreen');if(screen)screen.style.backgroundImage=`url('assets/${formal==='V3'?'shot08-v3.svg':'shot08-v4.svg'}')`;
      const progress=$('.clientProgress');if(progress){const delivered=!!(s.deliveryRecord||s.delivered),versionState=s.approval?`${s.approvedVersion||'V4'} Approved`:s.v4ChangesRequested?'V4 Changes Requested':s.v4Submitted?'V4 In Review':s.v3ChangesRequested?'V3 Changes Requested':s.v3Submitted?'V3 In Review':'Not submitted';progress.innerHTML=`<div class="step ${s.creativeApproval?'done':s.creativeSubmitted?'current':''}"><b>01 创意审阅</b><span>${s.creativeApproval?'Creative Approved':s.creativeSubmitted?'Waiting for decision':'Not published'}</span></div><div class="step ${s.creativeApproval&&!hasVersion?'current':hasVersion?'done':!s.creativeApproval?'locked':''}"><b>02 制作中</b><span>${s.creativeApproval?'Direction locked':'After creative approval'}</span></div><div class="step ${hasVersion&&!s.approval?'current':s.approval?'done':'locked'}"><b>03 成片审阅</b><span>${versionState}</span></div><div class="step ${s.approval&&!delivered?'current':delivered?'done':'locked'}"><b>04 最终交付</b><span>${delivered?'Delivered':s.approval?'Ready for delivery':'After version approval'}</span></div>`}
    }

    function render(){const s=state();project(s);producer(s);studio(s);director(s);review(s)}
    render();setTimeout(render,180);window.addEventListener('reelops:state',()=>setTimeout(render,160));setTimeout(()=>{if(sessionStorage.getItem('reelops_investor_demo_mode')==='1')document.querySelector('.demoGuide')?.classList.add('open')},420);
  });
})();
